...
with st.container():
    ...
    with col1:
        st.image("scr/Logo_Buenos_Aires.png", use_container_width=True)
        
    with col2:
        st.title("")
          
    with col3: 
        st.image("scr/SDTLogoC.png", use_container_width=True)
...
with st.container():
    ...
    with col1:
        st.header("Informe estadístico de accidentes viales en la Ciudad de Buenos Aires")        
        st.markdown("""
    ...
    with col2:
        st.image("scr/BA.png", use_container_width=True)

st.write("<hr style='border-top: 1px solid grey;'>", unsafe_allow_html=True)  

def main():    
    st.title("Dashboard de análisis de accidentes viales")
    st.markdown(
        """
